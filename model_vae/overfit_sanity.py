...
def main():
    ...
    ap.add_argument("--latent-view-count", type=int, default=-1,
                    help="Defaults to -1, meaning no view compression "
                         "(latent_view_count == view_count).")
    ap.add_argument("--temporal-downsample-factor", type=int, default=4)
    ap.add_argument("--temporal-pre", type=int, default=1)
    ...
    if args.latent_view_count <= 0:
        args.latent_view_count = args.view_count
    if args.latent_view_count > args.view_count:
        raise ValueError(
            "latent_view_count must be <= view_count for this sanity script."
        )
    ...
    print(
        ...
        f"latent_views={args.latent_view_count}/{args.view_count}"
    )
    ...
    if final_psnr > 28:
        ...
        first_psnr is not None
        and first_rec is not None
        and last_rec is not None
        and (final_psnr > first_psnr + 0.5 or last_rec < first_rec * 0.9)
    ):
        print(">> LEARNING: reconstruction is improving but has not reached "
              "18 dB yet. Run more steps for the full overfit check.")
    else:
