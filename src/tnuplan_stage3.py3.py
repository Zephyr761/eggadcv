"""
NuPlan仿真 - 阶段3：接入VAD Planner
目标：在阶段2基础上，接入VAD模型进行端到端规划
"""
import os
import numpy as np
...
print("=" * 80)
print("NuPlan仿真 - 阶段3：接入VAD Planner")
print("=" * 80)
...
def img_wconcat_save(tmp, gen_imgs_list, post_trans, auto_id, scene_id, save_dir):
    ...
    for bi, template in enumerate(tmp):
        for gen_id, gen_imgs in enumerate(gen_imgs_list):
            ...
            filename = Path(f"8view_{scene_id}_{auto_id}").with_suffix(".jpg")
            save_name = os.path.join(save_dir, filename)
            os.makedirs(os.path.dirname(save_name), exist_ok=True)
            img_cat.save(save_name)
            print(f"  保存生成图像: {save_name}")


class VADDiffusionSimulationRunner(SimulationRunner):
    """
    阶段3：集成VAD Planner的仿真运行器
    
    特点：
    1. 使用Diffusion生成图像
    2. 将生成的图像传递给VAD planner
    3. VAD planner输出规划轨迹
    """
    def __init__(
        self, 
        ...
        planner: VADPlanner,  # 明确指定VAD planner类型
        kernel_size=(1, 1),
        sigma=1.0,
        patch_radius=100,
        mean=[0.5, 0.5, 0.5],
    ...
    def get_diffused_img(self, planner_input: PlannerInput, prev_sensor_data: List[PILImage.Image], step):
        ...
        # 准备VAD输入
        vad_metas = VADPlannerInput(
            images=image,
            lidar2img=sensor_metas["lidar2image"],
            lidar2cam=sensor_metas["lidar2camera"],
    ...
    def run(self) -> RunnerReport:
        ...
        print(f"Planner: {self.planner.name()}")
        print(f"{'='*80}")
        ...
        global_count = 0
        max_steps = 40  # 阶段3运行40步
        
        print(f"\n开始仿真循环（最大步数：{max_steps}，每5步生成图像并使用VAD规划）...")
        ...
        while self.simulation.is_simulation_running():
            ...
            if counter == 4:
                if self.simulation._time_controller.get_iteration().index == 0:
                    ...
                    print(f"步骤 {global_count:3d}: 使用GT图像，VAD planner规划轨迹...")
                    ...
                    # 生成图像
                    print(f"步骤 {global_count:3d}: 生成图像并使用VAD planner规划...")
                    sensor_data, vad_input = self.get_diffused_img(planner_input, prev_sensor_data, planner_input.iteration.index)

                prev_sensor_data = sensor_data
                counter = 0

                # 传递VAD输入给planner
                self.planner.set_vad_input(vad_input)
                
                self._simulation.callback.on_planner_start(self.simulation.setup, self.planner)
                
                # 使用VAD planner规划轨迹
                print(f"  [VAD] 调用VAD planner计算轨迹...")
                trajectory = self.planner.compute_planner_trajectory(planner_input)
                ...
                print(f"步骤 {global_count:3d}: 位置=({current_pos[0]:6.2f}, {current_pos[1]:6.2f})，使用缓存轨迹")
        ...
        plt.plot(ego_trajectory_log[:, 0], ego_trajectory_log[:, 1], 
                marker='o', markersize=3, label='VAD Planner Trajectory', linewidth=2, color='purple')
        ...
        plt.title(f"Ego Vehicle Trajectory (Stage3 - VAD Planner) - {self._simulation.scenario.scenario_name}", fontsize=14)
        ...
        print(f"共执行 {global_count} 步，VAD规划 {global_count//5} 次")
        print(f"{'='*80}\n")
...
if __name__ == "__main__":
    print("\n开始运行NuPlan仿真（阶段3：接入VAD Planner）...\n")
    
    # 创建VAD planner
    vad_config = path_config.get('vad', {})
    vad_planner = VADPlanner(
        path_to_conf_file=vad_config.get('config_file', './vad_config.py'),
        ckpt_path=vad_config.get('checkpoint', './vad_checkpoint.pth'),
    )
    ...
    for idx, scenario in enumerate(scenarios[:1]):
        ...
        runner = VADDiffusionSimulationRunner(
            cfg.diffusion, simulation, vad_planner
        )
    ...
    print("="*80)
    print("\n总结：")
    print("1. stage3_output目录包含VAD planner的轨迹可视化")
    print("2. gen_img_log目录包含生成的图像")
    print("3. VAD planner已成功集成到仿真循环中")
    print("="*80)
