        ...
        for i in range(start_timestep, stop_timestep):
            ...
            if diffusion_forcing_mode:
                ...
                if frozen_ref_count > 0:
                    clean_idx = self.inference_config["inference_steps"] - 1 
                    timestep_indices[:, :frozen_ref_count, :] = clean_idx
            ...
            if diffusion_forcing_mode:
                ...
                base_schedule = torch.tensor([
                    i - j_item * steps_per_inference >= 0
                    for j_item in range(latent_shape[1])
                ], device=self.device)

                # ref 区：j < frozen_ref_count 不允许更新
                ref_mask = j >= frozen_ref_count

                in_schedule_1d = base_schedule & ref_mask
