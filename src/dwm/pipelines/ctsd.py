...
class CrossviewTemporalSD():
    ...
    def __init__(
        ...
        self.is_crossview_vae = getattr(self.vae, "is_crossview_vae", False) \
            or getattr(self.vae.config, "vae_type", None) == "crossview_4d"
    ...
    def get_latent_sequence_length(self, sequence_length):
        pre = self.inference_config.get(
            "vae_pre", getattr(self.vae.config, "temporal_pre", 0)
            if getattr(self, "is_crossview_vae", False) else 0)
        stride = self.inference_config.get(
            "vae_stride",
            getattr(self.vae.config, "temporal_downsample_factor", 1)
            if getattr(self, "is_crossview_vae", False) else 1)
    ...
    def get_vae_latent_view_count(self, view_count: int):
        return getattr(self.vae.config, "latent_view_count", view_count) \
            if self.is_crossview_vae else view_count

    def encode_vae_images(
        self, image_tensor, batch_size: int, sequence_length: int,
        view_count: int, shift_factor, sample: bool = True
    ):
        if self.is_crossview_vae:
            image_tensor = einops.rearrange(
                image_tensor, "(b t v) c h w -> b t v c h w",
                b=batch_size, t=sequence_length, v=view_count)
            latent_dist = self.vae.encode(image_tensor).latent_dist
            latents = latent_dist.sample() if sample else latent_dist.mode()
            return (latents - shift_factor) * self.vae.config.scaling_factor, \
                image_tensor
        ...
        latents = dwm.functional.memory_efficient_split_call(
            ...
            lambda block, tensor: (
                ...
                if sample else block.encode(tensor).latent_dist.mode()
            ) - shift_factor,
        ...
        if self.is_temporal_vae:
            ...
            latents = einops.rearrange(
                ...
                t=sequence_length, v=view_count)
        return latents, image_tensor

    def decode_vae_latents(self, latents, shift_factor):
        if self.is_crossview_vae:
            image_tensor = self.vae.decode(
                latents.to(dtype=self.vae.dtype) /
                self.vae.config.scaling_factor + shift_factor,
                return_dict=False)[0]
            return image_tensor.flatten(0, 2)
    ...
    def train_step(self, batch: dict, global_step: int):
        ...
        latents, image_tensor = self.encode_vae_images(
            image_tensor, batch_size, sequence_length, view_count,
            shift_factor, sample=True)
        ...
        with self.get_autocast_context():
            ...
            if self.training_config.get("extra_reference_infer", 0) > 0:
                ...
                extra_reference_infer = self.training_config.get(
                    "extra_reference_infer", 0)
                if self.is_crossview_vae:
                    extra_image_tensor = image_tensor[:, :extra_reference_infer]
                    extra_latents = (
                        self.vae.encode(extra_image_tensor).latent_dist.sample()
                        - shift_factor) * self.vae.config.scaling_factor
                elif self.is_temporal_vae:
                    extra_image_tensor = image_tensor[:, :, :extra_reference_infer]
    ...
    def inference_pipeline(
        ...
        if diffusion_forcing_mode:
            ...
            cur_latents = latents[:, take_time]
            if self.is_crossview_vae:
                image_tensor = self.decode_vae_latents(
                    cur_latents, shift_factor)
            else:
                cur_latents = cur_latents.flatten(0, 1).to(dtype=self.vae.dtype)
            if self.is_temporal_vae:
                view_count = latents.shape[2]
                ...
                image_tensor = einops.rearrange(
                    image_tensor, "(b v) c t h w -> (b t v) c h w", v=view_count)
            elif not self.is_crossview_vae:
                image_tensor = self.vae.decode(
                    cur_latents / self.vae.config.scaling_factor + shift_factor,
                    return_dict=False)[0]
            ...
            image_tensor = self.decode_vae_latents(latents, shift_factor)
    ...
    def autoregressive_inference_pipeline(
        ...
        if self.inference_config.get("generate_frames_for_reference", True):
            ...
            shift_factor = self.vae.config.shift_factor \
                if self.vae.config.shift_factor is not None else 0
            image_latents, _ = self.encode_vae_images(
                image_tensor, raw_image_tensor.shape[0],
                raw_image_tensor.shape[1], raw_image_tensor.shape[2],
                shift_factor, sample=False)
            # fix 4
        ...
        if "sequence_length_per_iteration" in self.inference_config:
            latent_shape = (
                ...
                self.get_latent_sequence_length(
                    self.inference_config["sequence_length_per_iteration"]),
                self.get_vae_latent_view_count(view_count),
            ...
            latent_shape = (
                batch_size, self.get_latent_sequence_length(
                    sequence_length), self.get_vae_latent_view_count(view_count),
                self.vae.config.latent_channels, latent_height,
                latent_width
            )
