...
def create_parser():
    ...
    parser.add_argument(
        "--checkpointing-steps", default=1000, type=int,
...
if __name__ == "__main__":
    ...
    if ddp:
        ...
        validation_datasampler = VariableVideoBatchSampler(
            validation_dataset,
            config["mix_config"],
            num_replicas=process_group.size(),
            rank=process_group.rank(),
        ...
        validation_dataloader = torch.utils.data.DataLoader(
            ...
            **dwm.common.instantiate_config(config["validation_dataloader"]),
            batch_sampler=validation_datasampler,
        )
    else:
        training_dataloader = torch.utils.data.DataLoader(
            training_dataset,
    ...
    for epoch in range(config["train_epochs"]):
        ...
        if ddp:
            ...
            validation_datasampler.set_epoch(sampler_epoch)
