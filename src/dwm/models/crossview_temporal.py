...
class AlphaBlender(torch.nn.Module): # 把a.b按照alpha进行融合的模块
    ...
    def __init__(
        ...
        if self.merge_strategy == "fixed":
            self.register_buffer("mix_factor", torch.Tensor([alpha])) # 固定的alpha值，作为一个buffer注册到模型中，不会被优化器更新。
        elif "learned" in self.merge_strategy: # alpha可学习的情况，作为一个参数注册到模型中，会被优化器更新。
    ...
    def get_alpha(self, image_only_indicator=None):
        if self.merge_strategy == "fixed":
            ...
            alpha = torch.sigmoid(self.mix_factor) # 保证alpha在0和1之间 是插值而不是外扩
            ...
            alpha = torch.where(
                ...
                torch.sigmoid(self.mix_factor))#类似c语言中的三目运算符
            # 是图像数据就把alpha设置为1 表示仅考虑当前数据
        else:
    ...
    def forward(
        ...
        """
        #这表示image_only_indicator表示一个batch内各样本输入的是不是图片
        alpha = self.get_alpha(image_only_indicator).to(a.dtype)
        expected_alpha_shape = list(alpha.shape) + \
            [1 for _ in range(len(a.shape) - len(alpha.shape))] #补齐维度方便之后广播
...
class ResBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.spatial_res_block = diffusers.models.resnet.ResnetBlock2D(
            ...
            temb_channels=temb_channels, eps=eps) # 空间维度的Resnet块，处理每个视角的特征图
...
class TemporalBasicTransformerBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        # 输入维度 与时间混合的内部维度 单头的维度 头的数量 外部注意力的维度
        super().__init__()
        self.is_res = dim == time_mix_inner_dim #如果输入输出维度相同则使用残差连接
    ...
    def forward(
        ...
        hidden_states = hidden_states.unflatten(0, (batch_size, -1))\
            .transpose(1, 2).flatten(0, 1) # [B * T * V, C] -> [B, C, T * V] -> [B * C, T * V]
        ...
        if self_attention_mask is not None: # 掩码要重复
